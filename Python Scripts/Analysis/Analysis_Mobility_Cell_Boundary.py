...
#Labels
Labels=['HarmonicMean', 'ArithmeticMean','Constant']

#List with characteristic lengths 
Lcs=[[] for _ in range(len(Labels))]

for M in range(len(Labels)):
    for I in range(Ns):
        Lc=[]
        Morphologies=np.load(Path+'\\'+Labels[M]+'\Morphologies'+str(I)+'.npy')
        for Morphology in Morphologies:
            Lc.append(Analyse_Morphology(Morphology))
        Lcs[M].append(Lc)

#Calculate average and standard deviation 
Lcs_av=[[] for _ in range(len(Labels))]
Lcs_std=[[] for _ in range(len(Labels))]
...
#%% Visualise evolution of the characteristic length for different means 
...
plt.errorbar(t_range, Lcs_av[2],Lcs_std[2],marker='o',markersize=1,linestyle=':',label='Constant',capsize=1)
plt.errorbar(t_range, Lcs_av[1],Lcs_std[1],marker='o',markersize=1,linestyle=':',label='Arithmetic',capsize=1)
plt.errorbar(t_range, Lcs_av[0],Lcs_std[0],marker='o',markersize=1,linestyle=':',label='Harmonic',capsize=1)
